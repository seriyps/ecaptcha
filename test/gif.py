# Source: https://github.com/xyRena/gif
...
class BitWriter(object):
    ...
    def write(self, num, pad=0):
        ...
        print("py write({}:{})".format(num, pad), file=sys.stderr)
        #--- Set up values ---
        ...
        while i > 0:
            ...
            if length <= byte_start:
                if self._bytes:
                    print("py push({})".format(self._bytes[-1]), file=sys.stderr)
            ...
            self._ptr += delta
            i = next_i
        # if length0 < len(self._bytes):
        #     print("py push({})".format([b for b in self._bytes[length0:]]), file=sys.stderr)
        return None
